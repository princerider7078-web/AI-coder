...
import { useOrders, ORDER_STATUS_LABELS, ORDER_STATUS_COLORS, type OrderStatus } from "@/contexts/OrdersContext";
    ...
    return (
      <Container className="py-16">
        <EmptyState
          icon={ShoppingBag}
          title="No orders yet"
          description="When you place your first order, it will appear here with full tracking details."
  ...
  return (
    <Container className="py-8 md:py-10">
      ...
      <div className="flex items-center gap-2 mb-6 overflow-x-auto pb-2">
        {FILTERS.map((f) => (
          <button key={f.value} onClick={() => setFilter(f.value)} className={cn("px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-colors", filter === f.value ? "bg-[#1A6B3C] text-white" : "bg-white text-slate-700 border border-slate-200 hover:border-[#1A6B3C]/30")}>
            {f.label}
          </button>
        ))}
      </div>
          ...
          filtered.map((order) => (
            <Link key={order.id} href={`/account/orders/${order.id}`} className="block bg-white border border-slate-200 rounded-xl p-4 hover:shadow-md hover:border-[#1A6B3C]/20 transition-all">
              ...
              <div className="flex items-center justify-between text-xs text-slate-500">
                <span>{order.items.length} {order.items.length === 1 ? "item" : "items"} · {order.paymentMethod === "cod" ? "COD" : "Paid Online"}</span>
